...
(function(){
  ...
  function questionHtml(question,label){return `<section class="math-card"><div class="math-kicker">${esc(label)}</div><h2>${esc(question.question)}</h2><div class="math-options">${question.choices.map((choice,index)=>`<button type="button" class="math-choice${mathState.selectedAnswer===choice?' selected':''}" data-math-action="select" data-choice-index="${index}">${index+1}. ${esc(choice)}</button>`).join('')}</div><div class="math-actions"><button type="button" class="math-primary" data-math-action="submit"${mathState.selectedAnswer===null?' disabled aria-disabled="true"':''}>답 제출하기</button></div></section>`;}
