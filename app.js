...
const MATH_LEARNING_ACCESS_TYPES=[
  ...
  {key:'appliedConcept',label:'응용개념학습'}
];
...
async function apiGetStudyTime(name){
  ...
  try{
    ...
    if(result?.ok&&result.skipped!==true&&typeof rememberConfirmedMathProgress_==='function')rememberConfirmedMathProgress_(snapshot);
    return result;
...
document.addEventListener('click',function(event){
  ...
  const dueDate=screen.dataset.homeworkDueDate||'';
  if(!isHomeworkPastDue_(playerName,dueDate))return;
  event.preventDefault();event.stopImmediatePropagation();showToast2('🔒 오늘 학습 시간이 종료되었습니다.');
},true);
...
function selectUnit(el,key){
  ...
  currentUnit=key;
  const unitWasCompleted=(UNITS[key].examMode?calculateSummaryQuizProgress(playerName,key):calculateUnitProgress(playerName,key)).completed===true;
  if(unitWasCompleted&&!UNITS[key].examMode){
    currentLevel='easy';
    document.querySelectorAll('.level-card').forEach(card=>card.classList.toggle('active',card.dataset.level==='easy'));
  }
...
function createMapStudyCard(){
  ...
  const completed=playerName&&calculateMapStudyProgress(playerName).completed===true;
  card.className='unit-card'+(completed?' done':'');
  card.dataset.learningContent='mapStudy';
  card.innerHTML=`<div class="unit-icon">🗺️</div>
    <div class="unit-info">
      <div class="unit-title">지도 문제</div>
      <div class="unit-sub">${completed?'완료 · 다시 학습하기':'PART 1~5 · 위치와 영토 익히기 · 50문제'}</div>
    </div>`;
...
async function showMapStudyList(){
  ...
  document.getElementById('map-part-list').innerHTML=visibleMapParts.map(part=>{
    ...
    return `<button class="map-part-card" onclick="openMapStudyLearn('${part.id}')">
      ...
      <span class="map-part-score">${p?.passed?'완료 · 다시 학습하기':score}</span>
    </button>`;
  }).join('');
  window.scrollTo(0,0);
}
...
function createUnitCardElement(key){
  ...
  const completed=(u.examMode?calculateSummaryQuizProgress(playerName,key):calculateUnitProgress(playerName,key)).completed===true;
  c.innerHTML=`<div class="unit-icon">${u.icon}</div>
    <div class="unit-info">
      <div class="unit-title">${u.title}</div>
      <div class="unit-sub">${!approved?'선생님 공개 대기 중':(completed?'완료 · 다시 학습하기':(ready?(u.examMode?`${u.totalQuestions}문제 · ${Math.floor(u.duration/60)}분`:`기본 ${u.easyCount} · 심화 ${u.hardCount}`):'문제 준비 중 · 문제 추가 시 자동 활성화'))}</div>
    </div>
...
const LEARNING_MODULES={
  ...
  mathBasicConceptReview:{
    key:'mathBasicConceptReview', title:'수학 기본개념 확인', includeInOverall:true,
  ...
  mathConceptExtension:{
    ...
    calculateProgress:calculateMathConceptExtensionProgress,
    getIncompleteItems:getMathConceptExtensionIncompleteItems,
    getResumeTarget:getMathConceptExtensionResumeTarget
  },
  mathAppliedConcept:{
    key:'mathAppliedConcept', title:'수학 응용개념학습', includeInOverall:true,
    calculateProgress:calculateMathAppliedConceptProgress,
    getIncompleteItems:getMathAppliedConceptIncompleteItems,
    getResumeTarget:getMathAppliedConceptResumeTarget
  },
  mathConceptReviewLearning:{
    key:'mathConceptReviewLearning', title:'수학 개념복습학습', includeInOverall:true,
    calculateProgress:calculateMathConceptReviewLearningProgress,
    getIncompleteItems:getMathConceptReviewLearningIncompleteItems,
    getResumeTarget:getMathConceptReviewLearningResumeTarget
  },
  mathWrongPractice:{
    key:'mathWrongPractice', title:'수학 오답연습', includeInOverall:true,
...
function calculateOverallProgressV2(name){
  ...
  // 서버 응답 순서와 화면 순서는 분리한다. 현재 필수 수학이 있으면 수학을 먼저,
  // 수학이 없을 때만 역사가 첫 항목이 되도록 한 뒤 과목 안에서는 기존 순서를 유지한다.
  const mathModuleKeys=new Set(['mathConcept','mathBasicConceptReview','mathConceptReviewLearning','mathWrongPractice']);
  const sortedItems=requiredItems.filter(item=>!item.completed&&item.displayItem).map(item=>item.displayItem).sort((a,b)=>{
    const subjectOrderA=mathModuleKeys.has(a.moduleKey)?0:1;
    const subjectOrderB=mathModuleKeys.has(b.moduleKey)?0:1;
    return subjectOrderA-subjectOrderB||(a.sortOrder||0)-(b.sortOrder||0);
  });
  const seen=new Set();
  const dedupedItems=[];
  ...
  if(USE_UNIFIED_PROGRESS){
    try{
      normalizedItems=buildIncompleteLearningItemsV2(playerName).map(it=>({
        ...
        locked:it.locked===true,lockMessage:it.lockMessage||'',
  ...
  body.innerHTML=normalizedItems.map(n=>{
    ...
    return `<div class="unit-card ht-incomplete-card${locked?' teacher-locked-content':''}" aria-disabled="${locked?'true':'false'}">
      <div class="unit-info">
        ...
        <div class="unit-title">${isMathSequential?`<span class="incomplete-sequence-number">${sequenceNumber}.</span> `:''}${n.title}</div>
        ${isReview
...
function getHomeworkDueDateForTarget_(target){
  if(!target)return '';
  const content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT,unitId=target.unitId||target.unitKey||'';
  if(['mathConcept','mathBasicConceptReview','mathConceptReviewLearning','mathWrongPractice'].includes(target.type))return content?.units?.[unitId]?.scheduledDate||'';
  if(target.type==='unit')return deadlineMap?.[target.unitKey]||'';
...
function handleResumeLearningElement(element,event){
  ...
  if(element?.dataset?.locked==='true'||element?.disabled){
    if(typeof showToast2==='function')showToast2('🔒 '+(element?.dataset?.lockMessage||'이전 학습을 먼저 완료해 주세요.'));
    return;
  }
...
function resolveLearningResumeTarget(el){
  ...
  if(type==='mathConceptExtension') return {type:'mathConceptExtension',unitId:el.dataset.unitId||''};
  if(type==='mathAppliedConcept') return {type:'mathAppliedConcept',unitId:el.dataset.unitId||''};
  if(type==='mathConceptReviewLearning') return {type:'mathConceptReviewLearning',unitId:el.dataset.unitId||''};
  if(type==='mathBasicConceptReview') return {type:'mathBasicConceptReview',unitId:el.dataset.unitId||''};
...
function openLearningResumeTarget(target){
  ...
  }else if(target.type==='mathConceptExtension'){
    openMathConceptLearningExtension(target.unitId||'');
  }else if(target.type==='mathAppliedConcept'){
    openMathAppliedConceptLearning(target.unitId||'');
  }else if(target.type==='mathConceptReviewLearning'){
    openMathConceptReviewLearning(target.unitId||'');
...
function rememberConfirmedMathProgress_(snapshot){
  if(!snapshot||!snapshot.studentKey||!snapshot.unitId)return;
  mathServerConfirmedProgressCache[mathConceptCacheKey_(snapshot.studentKey,snapshot.unitId)]=JSON.parse(JSON.stringify(snapshot));
}

// 수학 엔진의 서버 저장 성공 직후 홈 진행률/미완료 목록도 같은 확정 스냅샷을 보게 한다.
window.onMathConceptProgressServerSaved=function(snapshot){
  if(!snapshot||!snapshot.studentKey||!snapshot.unitId||isLearningWriteBlocked())return;
  const previous=mathServerConfirmedProgressCache[mathConceptCacheKey_(snapshot.studentKey,snapshot.unitId)],firstCompletion=snapshot.completed===true&&previous?.completed!==true;
...
// 전체 진행률은 서버에서 복원된 확정 기록만 사용한다. 관리자 미리보기나 아직
// 동기화되지 않은 브라우저 임시 상태가 학생의 실제 진행률에 섞이지 않게 한다.
function getServerMathConceptProgress_(name,unitId){
...
function getMathSequentialLock_(name,targetUnitId,content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT){
  ...
  const targetProgress=pickNewerMathConceptProgress_(readMathConceptProgressWithMigration_(name,targetUnitId),mathConceptProgressOverviewCache[mathConceptCacheKey_(name,targetUnitId)]);
  if(targetProgress?.studentKey===name&&targetProgress?.unitId===targetUnitId&&targetProgress.completed===true)return {locked:false,blocker:null};
  for(const unitId of unitIds.slice(0,targetIndex)){
    const progress=pickNewerMathConceptProgress_(readMathConceptProgressWithMigration_(name,unitId),mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]),unit=content?.units?.[unitId];
    if(!progress||progress.studentKey!==name||progress.unitId!==unitId||progress.completed!==true)return {locked:true,blocker:{unitId,title:unit?.title||MATH_BASIC_REVIEW_UNIT_TITLES[unitId]||unitId,type:'mathConcept'}};
    if(Array.isArray(unit?.basicConceptReview)&&unit.basicConceptReview.length&&progress.basicConceptReview?.completed!==true)return {locked:true,blocker:{unitId,title:unit.title||unitId,type:'mathBasicConceptReview'}};
    if(unit?.conceptReviewLearning&&progress.conceptReviewLearning?.completed!==true)return {locked:true,blocker:{unitId,title:unit.title||unitId,type:'mathConceptReviewLearning'}};
  }
...
function calculateMathConceptProgress(name){
  ...
  const completedCount=unitIds.filter(unitId=>{
    const raw=mathServerConfirmedProgressCache[mathConceptCacheKey_(name,unitId)],expectedVersion=Number(content.units[unitId].contentVersion)||2;
    return !!(raw&&Number(raw.contentVersion)===expectedVersion&&raw.completed===true);
  }).length;
  const total=unitIds.length,incompleteCount=total-completedCount,completed=incompleteCount===0;
  const percent=Math.round(completedCount/total*100);
  const nextUnitId=unitIds.find(unitId=>mathServerConfirmedProgressCache[mathConceptCacheKey_(name,unitId)]?.completed!==true)||unitIds[0];
  const sequentialLock=!completed?getMathSequentialLock_(name,nextUnitId,content):{locked:false};
  return {
    percent, completed,
    status: completed?'완료':`미완료 ${incompleteCount}개`,
    locked:sequentialLock.locked,lockMessage:sequentialLock.locked?'이전 학습을 먼저 완료해 주세요':'',
    incompleteCount,
    resumeTarget: completed?null:{type:'mathConcept',unitId:nextUnitId,unitKey:nextUnitId},
    completedAmount:completedCount, totalAmount:total,
    includeInOverall:true
  };
}
...
async function loadMathConceptProgressForName_(name){
  ...
  await Promise.all(unitIds.map(async unitId=>{
    ...
    rememberConfirmedMathProgress_(server);
...
function getMathBasicReviewScheduledDate_(unitId,content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT){return content?.units?.[unitId]?.scheduledDate||'';}
function isMathBasicReviewScheduleOpen_(unitId,now=new Date(),content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT){const scheduledDay=mathLocalDayNumber_(getMathBasicReviewScheduledDate_(unitId,content)),today=mathLocalDayNumber_(now);return scheduledDay!==null&&today!==null&&today>scheduledDay;}
function isMathBasicReviewRetroactive_(progress,unitId,now=new Date(),content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT){return progress?.completed===true&&!hasMathBasicReviewRecord_(progress)&&!progress.basicConceptReviewPolicyVersion&&MATH_BASIC_REVIEW_RETROACTIVE_UNITS.has(unitId)&&isMathBasicReviewScheduleOpen_(unitId,now,content);}
function isMathBasicReviewEligible_(progress,now=new Date(),unitId=progress?.unitId,content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT){
  if(progress?.completed!==true)return false;
  if(hasMathBasicReviewRecord_(progress))return true;
  return isMathBasicReviewScheduleOpen_(unitId,now,content);
}
...
function getMathBasicConceptReviewResumeTarget(name){return getMathBasicConceptReviewIncompleteItems(name)[0]?.resumeTarget||null;}

// 완료된 단원에 새로 추가된 문항("새 학습 이어하기")만 별도로 추적하는 형제 모듈.
// mathConcept 모듈 자체의 완료 여부/완료시각/진행률은 전혀 건드리지 않고, 새 문항 유무만 독립적으로 판정한다.
function calculateMathConceptExtensionProgress(name){
  const student=STUDENTS.find(item=>item.name===name),content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT;
  const units=student&&content?getAssignedMathUnitIds_(student,content).filter(unitId=>getServerMathConceptProgress_(name,unitId)?.completed===true):[];
  const total=units.length,completedCount=units.filter(unitId=>getMathUnitNewItemsCount_(name,unitId)===0).length,incompleteCount=total-completedCount;
  return {percent:total?Math.round(completedCount/total*100):100,completed:incompleteCount===0,status:incompleteCount?`미완료 ${incompleteCount}개`:'완료',incompleteCount,
    resumeTarget:getMathConceptExtensionIncompleteItems(name)[0]?.resumeTarget||null,completedAmount:completedCount,totalAmount:total,includeInOverall:total>0};
}
function getMathConceptExtensionIncompleteItems(name){
  const student=STUDENTS.find(item=>item.name===name),content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT;if(!student||!content)return [];
  return getAssignedMathUnitIds_(student,content).map((unitId,index)=>{
    if(getServerMathConceptProgress_(name,unitId)?.completed!==true)return null;
    const newCount=getMathUnitNewItemsCount_(name,unitId);if(newCount<=0)return null;
    const unit=content.units?.[unitId],lock=getMathSequentialLock_(name,unitId,content);
    return {moduleKey:'mathConceptExtension',itemKey:unitId,title:`수학개념학습 · ${unit?.title||unitId}`,subtitle:'수학',percent:0,status:`새 학습 ${newCount}문제`,locked:lock.locked,lockMessage:lock.locked?'이전 학습 완료 후 가능':'',resumeTarget:{type:'mathConceptExtension',unitId,unitKey:unitId},sortOrder:1003+(index*3)+1};
  }).filter(Boolean);
}
...
// 응용개념학습 — 기본학습(mathConcept)과 완전히 분리된 진행/완료 상태(progress.appliedConcepts)로 추적하는 형제 모듈.
function getMathAppliedConceptIncompleteItems(name){
  const student=STUDENTS.find(item=>item.name===name),content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT;if(!student||!content)return [];
  return getAssignedMathUnitIds_(student,content).map((unitId,index)=>{
    const unit=content.units?.[unitId];if(!unit?.appliedConcepts)return null;
    const progress=getServerMathConceptProgress_(name,unitId);
    if(progress?.appliedConcepts?.completed===true)return null;
    const baseCompleted=progress?.studentKey===name&&progress?.unitId===unitId&&progress.completed===true,priorLocked=getMathSequentialLock_(name,unitId,content).locked,locked=priorLocked||!baseCompleted;
    return {moduleKey:'mathAppliedConcept',itemKey:unitId,title:`수학 응용개념학습 · ${unit.title}`,subtitle:'수학',percent:0,status:locked?'이전 학습 완료 후 가능':'학습하기',locked,lockMessage:locked?'이전 학습 완료 후 가능':'',resumeTarget:{type:'mathAppliedConcept',unitId,unitKey:unitId},sortOrder:1006+(index*3)};
  }).filter(Boolean);
}
function calculateMathAppliedConceptProgress(name){const student=STUDENTS.find(item=>item.name===name),content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT,units=student&&content?getAssignedMathUnitIds_(student,content).filter(unitId=>content.units?.[unitId]?.appliedConcepts):[],total=units.length,completedCount=units.filter(unitId=>mathServerConfirmedProgressCache[mathConceptCacheKey_(name,unitId)]?.appliedConcepts?.completed===true).length,incompleteCount=total-completedCount,items=getMathAppliedConceptIncompleteItems(name);return {percent:total?Math.round(completedCount/total*100):100,completed:incompleteCount===0,status:incompleteCount?`미완료 ${incompleteCount}개`:'완료',incompleteCount,resumeTarget:items.find(item=>!item.locked)?.resumeTarget||null,completedAmount:completedCount,totalAmount:total,includeInOverall:total>0};}
function getMathAppliedConceptResumeTarget(name){return getMathAppliedConceptIncompleteItems(name).find(item=>!item.locked)?.resumeTarget||null;}
...
function findMathWrongConceptId_(unit,questionId){
  ...
  const requiredPrerequisite=(unit.requiredPrerequisiteChecks||[]).find(item=>item.question?.id===questionId||item.retryQuestion?.id===questionId);
  if(requiredPrerequisite)return requiredPrerequisite.id;
...
function getMathWrongConceptReview_(item){
  ...
  const requiredPrerequisite=(unit.requiredPrerequisiteChecks||[]).find(concept=>concept.id===conceptId);
  if(requiredPrerequisite){
    const review=requiredPrerequisite.review||{};
    return {conceptId,title:requiredPrerequisite.title,summary:review.summary||'',keyPoint:review.keyPoint||'',example:review.example||'',diagram:review.diagram||''};
  }
...
function getMathWrongPracticeItems_(name){
  ...
  getMathUnitIds_(student,content).forEach(unitId=>{
    // 오답연습 목록과 진행률은 서버에서 복원된 확정 기록만 사용한다.
    const progress=mathServerConfirmedProgressCache[mathConceptCacheKey_(name,unitId)];
...
function getMathWrongPracticeIncompleteItems(name){
  ...
  return progress.units.filter(unit=>unit.remaining>0).map((unit,index)=>({moduleKey:'mathWrongPractice',itemKey:unit.unitId,title:`수학 오답연습 · ${unit.title} · ${unit.remaining}문제 남음`,subtitle:'수학',
    percent:0,status:`${unit.remaining}문제 남음`,resumeTarget:{type:'mathWrongPractice',unitId:unit.unitId},sortOrder:1006+(index*3)}));
}
...
async function renderMathLearningCards_(){
  ...
  const appliedTeacherLocked=!isMathLearningAccessOpen_(playerName,activeUnitId,'appliedConcept');
  ...
  // 완료된 단원에 새 문항이 추가됐는지를 카드 부제에만 반영한다(진행 상태 자체는 여전히 math-flow-v2.js가 관리).
  const activeNewCount=activeUnit&&activeProgress.completed?getMathUnitNewItemsCount_(playerName,activeUnitId):0;
  const newItemsLabel=activeNewCount>0?`완료 · 새 학습 ${activeNewCount}문제`:'완료 · 다시 학습하기';
  if(sub)sub.textContent=activeUnit?((activeUnit.gradeLabel||'')+' · '+(midnightLocked?'오늘 학습 시간이 종료되었습니다.':conceptTeacherLocked?'선생님이 잠근 학습입니다.':activeLock.locked?'이전 학습을 먼저 완료해 주세요':(activeProgress.completed?newItemsLabel:'선수개념부터 차근차근 이해하기'))):'콘텐츠 준비 중';
  ...
  // 응용개념학습 — 기본학습과 별개 완료 상태(appliedConcepts)로 노출되는 새 카드(기존 카드/기본학습에는 포함하지 않음)
  const appliedCard=document.getElementById('math-applied-concept-card'),appliedTitle=document.getElementById('math-applied-concept-title'),appliedSub=document.getElementById('math-applied-concept-sub');
  const appliedProgress=pickNewerMathConceptProgress_(readMathConceptProgressWithMigration_(playerName,activeUnitId),mathConceptProgressOverviewCache[mathConceptCacheKey_(playerName,activeUnitId)]),appliedCompleted=appliedProgress?.appliedConcepts?.completed===true,appliedReady=!!(activeUnit?.appliedConcepts&&(appliedCompleted||(activeProgress.completed&&!activeLock.locked)));
...
function openMathBasicConceptReviewForActive_(){
  const student=STUDENTS.find(item=>item.name===playerName),content=window.MATH_CONTENT||window.MATH_CONCEPT_CONTENT,unitId=getMathActiveUnitId_(student,content),item=getMathBasicConceptReviewUnits_(playerName).find(entry=>entry.unitId===unitId);
  if(!item){showToast2('아직 기본개념 확인 시간이 아니에요.');return;}
  openMathBasicConceptReview(unitId);
}
...
function getResolvedMathWrongPracticeItems_(name){
  ...
  getMathUnitIds_(student,content).forEach(unitId=>{
    const progress=getServerMathConceptProgress_(name,unitId),unit=content.units?.[unitId];
    Object.values(progress?.wrongPractice?.items||{}).forEach(item=>{if(item?.resolved===true&&item.unitId===unitId)items.push({...item,unitTitle:unit?.title||unitId});});
  });
  return items;
}
...
function renderMathWrongPractice_(){
  ...
  if(!mathWrongPracticeQueue.length){
    ...
    root.innerHTML=`<section class="math-card"><div class="math-kicker">수학 오답연습</div><h2>모든 오답을 해결했어요!</h2><p>${replayAvailable?'완료 기록은 유지한 채 다시 연습할 수 있어요.':'새 오답이 생기면 이곳에서 연습할 수 있어요.'}</p><div class="math-actions">${replayAvailable?'<button type="button" class="math-primary" onclick="openMathWrongPractice()">다시 학습하기</button>':''}<button type="button" class="math-secondary" onclick="closeMathConceptLearning()">학습 홈으로</button></div></section>`;
...
let mathConceptReviewUnit=null,mathConceptReviewItems=[],mathConceptReviewState=null,mathConceptReviewAnswer='';
...
async function openMathConceptReviewLearning(unitId=''){
  ...
  if(__card){ if(__card.classList.contains('tap-pending'))return; __card.classList.add('tap-pending'); }
  try{
    const content=await loadMathConceptContent(),student=STUDENTS.find(item=>item.name===playerName);
    // 진행 상태는 로그인 시 이미 캐시돼 있으므로 재조회를 기다리지 않고 화면부터 연다 — 최신값은 백그라운드로만 확인.
...
function renderMathConceptReviewLearning_(){
  const root=document.getElementById('math-concept-root'),step=document.getElementById('math-step-label');if(!root||!mathConceptReviewUnit)return;
  const concepts=getMathConceptReviewConcepts_();
  if(mathConceptReviewState.phase==='completed'){
    if(step)step.textContent='개념복습학습 완료';root.innerHTML=`<section class="math-card"><div class="math-kicker">${mathEscape(mathConceptReviewUnit.title)} · 개념복습학습</div><h2>핵심개념 복습을 완료했어요!</h2><p>완료 기록은 유지돼요. 필요하면 처음부터 다시 복습할 수 있어요.</p><div class="math-actions"><button type="button" class="math-primary" onclick="restartMathConceptReviewLearning()">다시 복습하기</button><button type="button" class="math-secondary" onclick="closeMathConceptLearning()">학습 홈으로</button></div></section>`;renderMathLearningCards_();return;
  }
...
async function nextMathConceptReviewConcept(){const concepts=getMathConceptReviewConcepts_();if(mathConceptReviewState.conceptIndex<concepts.length-1)mathConceptReviewState.conceptIndex++;else{mathConceptReviewState.phase='questions';mathConceptReviewState.questionIndex=0;}await saveMathConceptReviewLearning_();renderMathConceptReviewLearning_();}
function selectMathConceptReviewAnswer(index){const item=mathConceptReviewItems[mathConceptReviewState.questionIndex],result=mathConceptReviewState.results[item.question.id]||{},question=result.needsReview?item.retryQuestion:item.question;mathConceptReviewAnswer=question.choices[index];renderMathConceptReviewLearning_();}
function startMathConceptReviewRetry(){mathConceptReviewState.phase='questions';mathConceptReviewAnswer='';renderMathConceptReviewLearning_();}
async function submitMathConceptReviewAnswer(){
...
async function saveMathConceptReviewLearning_(wrong=null){
  if(isLearningWriteBlocked())return false;
  const name=playerName,unitId=mathConceptReviewUnit.id;let snapshot=pickNewerMathConceptProgress_(readMathConceptProgressWithMigration_(name,unitId),mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]);if(!snapshot||snapshot.studentKey!==name||snapshot.unitId!==unitId)return false;
  snapshot=JSON.parse(JSON.stringify(snapshot));snapshot.conceptReviewLearning=JSON.parse(JSON.stringify(mathConceptReviewState));snapshot.wrongPractice={...(snapshot.wrongPractice||{}),items:{...(snapshot.wrongPractice?.items||{})}};
  if(wrong){const q=wrong.question,old=snapshot.wrongPractice.items[q.id]||{};snapshot.wrongPractice.items[q.id]={...old,questionId:q.id,unitId,conceptId:wrong.item.conceptId,question:q.question,choices:Array.isArray(q.choices)?q.choices.slice():[],correctAnswer:String(q.correctAnswer),acceptedAnswers:Array.isArray(q.acceptedAnswers)?q.acceptedAnswers.slice():[],diagram:String(q.diagram||''),studentAnswer:wrong.answer,explanation:String(q.explanation||''),source:'concept-review-learning',wrongAt:wrong.attemptedAt,resolved:false,resolvedAt:null};}
  snapshot.syncRevision=(Number(snapshot.syncRevision)||0)+1;snapshot.updatedAt=new Date().toISOString();snapshot.pendingSync=true;try{localStorage.setItem(mathConceptUnitStorageKey_(name,unitId),JSON.stringify(snapshot));}catch(error){}mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;
  for(let attempt=0;attempt<2;attempt++){const result=await apiSaveMathConceptProgress(snapshot);if(result?.ok){snapshot.lastServerRevision=Math.max(Number(snapshot.lastServerRevision)||0,Number(result.savedRevision)||snapshot.syncRevision);snapshot.pendingSync=snapshot.syncRevision>snapshot.lastServerRevision;try{localStorage.setItem(mathConceptUnitStorageKey_(name,unitId),JSON.stringify(snapshot));}catch(error){}mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;renderIncompleteUnitsSection();renderMathLearningCards_();return true;}if(result?.error!=='STALE_REVISION'||!result.data||result.data.unitId!==unitId)break;const server=result.data;snapshot={...server,conceptReviewLearning:JSON.parse(JSON.stringify(mathConceptReviewState)),wrongPractice:{...(server.wrongPractice||{}),items:{...(server.wrongPractice?.items||{}),...(snapshot.wrongPractice?.items||{})}},syncRevision:Math.max(Number(result.savedRevision)||0,Number(server.syncRevision)||0)+1,updatedAt:new Date().toISOString(),pendingSync:true};}
  mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;renderIncompleteUnitsSection();renderMathLearningCards_();return false;
}

// ── 응용개념 학습 ── 기존 기본학습(math-flow-v2.js, 선수개념/핵심개념/최종퀴즈)과 완전히 분리된 별도 학습.
// conceptReviewLearning과 같은 구조(app.js 자체 상태+렌더/제출, progress에 별도 키로 저장)를 그대로 따르되,
// coreConcepts를 재사용하는 "핵심개념 짧은 복습" 단계 대신 각 문항 자체의 lesson을 문제와 함께 보여준다.
let mathAppliedConceptUnit=null,mathAppliedConceptItems=[],mathAppliedConceptState=null,mathAppliedConceptAnswer='';
function defaultMathAppliedConceptState_(){return {startedAt:new Date().toISOString(),completed:false,completedAt:null,firstCompletedAt:null,relearning:false,phase:'questions',questionIndex:0,results:{},history:[]};}
function mathAppliedConceptQuestionHtml_(question){
  const choices=Array.isArray(question.choices)?question.choices.map((choice,index)=>`<button type="button" class="math-choice${mathAppliedConceptAnswer===choice?' selected':''}" onclick="selectMathAppliedConceptAnswer(${index})">${index+1}. ${mathFractionizeText_(choice)}</button>`).join(''):'';
  const input=choices?'':(mathFractionInputHtml_(question,mathAppliedConceptAnswer,'mathAppliedConceptAnswer')||`<input class="math-answer" inputmode="text" autocomplete="off" placeholder="정답을 직접 입력하세요" value="${mathEscape(mathAppliedConceptAnswer)}" oninput="mathAppliedConceptAnswer=this.value">`);
  return `<h2>${mathFractionizeText_(question.question)}</h2>${question.diagram?`<div class="math-diagram">${question.diagram}</div>`:''}<div class="math-options">${choices||input}</div><div class="math-actions"><button type="button" class="math-primary" onclick="submitMathAppliedConceptAnswer()">답 확인하기</button></div>`;
}
async function openMathAppliedConceptLearning(unitId=''){
  if(!playerName)return;
  if(blockStudentLearningEntry_())return;
  const __card=document.getElementById('math-applied-concept-card');
  if(__card){ if(__card.classList.contains('tap-pending'))return; __card.classList.add('tap-pending'); }
  try{
    const content=await loadMathConceptContent(),student=STUDENTS.find(item=>item.name===playerName);
    // 진행 상태는 로그인 시 이미 캐시돼 있으므로 재조회를 기다리지 않고 화면부터 연다 — 최신값은 백그라운드로만 확인.
    loadMathConceptProgressForName_(playerName).catch(error=>console.warn('응용개념학습 진행 상태 백그라운드 갱신 실패:',error));
    unitId=unitId||getMathActiveUnitId_(student,content);const unit=content.units?.[unitId],progress=pickNewerMathConceptProgress_(readMathConceptProgressWithMigration_(playerName,unitId),mathConceptProgressOverviewCache[mathConceptCacheKey_(playerName,unitId)]);
    if(blockMathLearningAccess_(playerName,unitId,'appliedConcept'))return;
    if(isHomeworkPastDue_(playerName,unit?.scheduledDate||'')){showToast2('🔒 오늘 학습 시간이 종료되었습니다.');return;}
    const appliedScreen=document.getElementById('math-concept-screen');if(appliedScreen)appliedScreen.dataset.homeworkDueDate=unit?.scheduledDate||'';
...
function renderMathAppliedConceptLearning_(){
  const root=document.getElementById('math-concept-root'),step=document.getElementById('math-step-label');if(!root||!mathAppliedConceptUnit)return;
  if(mathAppliedConceptState.phase==='completed'){
    if(step)step.textContent='응용개념학습 완료';root.innerHTML=`<section class="math-card"><div class="math-kicker">${mathEscape(mathAppliedConceptUnit.title)} · 응용개념학습</div><h2>응용개념 학습을 완료했어요!</h2><p>완료 기록은 유지돼요. 필요하면 처음부터 다시 학습할 수 있어요.</p><div class="math-actions"><button type="button" class="math-primary" onclick="restartMathAppliedConceptLearning()">다시 학습하기</button><button type="button" class="math-secondary" onclick="closeMathConceptLearning()">학습 홈으로</button></div></section>`;renderMathLearningCards_();return;
  }
  const item=mathAppliedConceptItems[mathAppliedConceptState.questionIndex],result=mathAppliedConceptState.results[item?.question?.id]||{};
  if(mathAppliedConceptState.phase==='review'){
    const lesson=item.lesson||{};if(step)step.textContent=`상황·식 다시 보기 · ${mathAppliedConceptState.questionIndex+1} / ${mathAppliedConceptItems.length}`;
    root.innerHTML=`<section class="math-card"><div class="math-kicker">틀린 부분 다시 보기</div><h2>${mathEscape(item.title)}</h2>${lesson.summary?`<p>${mathFractionizeText_(lesson.summary)}</p>`:''}${lesson.keyPoint?`<p><strong>${mathFractionizeText_(lesson.keyPoint)}</strong></p>`:''}${lesson.example?`<div class="math-equation">${mathFractionizeText_(lesson.example)}</div>`:''}<div class="math-actions"><button type="button" class="math-primary" onclick="startMathAppliedConceptRetry()">조건이 다른 문제로 다시 확인하기 →</button></div></section>`;return;
  }
  const question=result.needsReview?item.retryQuestion:item.question,lesson=item.lesson||{};if(step)step.textContent=`${item.stage} · ${mathAppliedConceptState.questionIndex+1} / ${mathAppliedConceptItems.length}`;
  root.innerHTML=`<section class="math-card"><div class="math-kicker">${mathEscape(item.stage)} · ${mathEscape(item.title)}</div>${lesson.summary?`<p>${mathFractionizeText_(lesson.summary)}</p>`:''}${lesson.keyPoint?`<p><strong>${mathFractionizeText_(lesson.keyPoint)}</strong></p>`:''}${mathAppliedConceptQuestionHtml_(question)}</section>`;
}
function selectMathAppliedConceptAnswer(index){const item=mathAppliedConceptItems[mathAppliedConceptState.questionIndex],result=mathAppliedConceptState.results[item.question.id]||{},question=result.needsReview?item.retryQuestion:item.question;mathAppliedConceptAnswer=question.choices[index];renderMathAppliedConceptLearning_();}
function startMathAppliedConceptRetry(){mathAppliedConceptState.phase='questions';mathAppliedConceptAnswer='';renderMathAppliedConceptLearning_();}
async function submitMathAppliedConceptAnswer(){
  if(blockExpiredHomeworkStep_(mathAppliedConceptUnit?.scheduledDate||''))return;
  const item=mathAppliedConceptItems[mathAppliedConceptState.questionIndex];if(!item||String(mathAppliedConceptAnswer).trim()==='')return;const previous=mathAppliedConceptState.results[item.question.id]||{},question=previous.needsReview?item.retryQuestion:item.question,correct=isMathAnswerCorrect(question,mathAppliedConceptAnswer),attemptedAt=new Date().toISOString();
  mathAppliedConceptState.results[item.question.id]={...previous,passed:correct,needsReview:!correct,attempts:[...(previous.attempts||[]),{questionId:question.id,answer:String(mathAppliedConceptAnswer),correct,attemptedAt}]};
  if(!correct){mathAppliedConceptState.phase='review';}else if(mathAppliedConceptState.questionIndex<mathAppliedConceptItems.length-1){mathAppliedConceptState.questionIndex++;mathAppliedConceptState.phase='questions';}else{const firstCompletion=!mathAppliedConceptState.completed;mathAppliedConceptState.completed=true;mathAppliedConceptState.completedAt=mathAppliedConceptState.completedAt||attemptedAt;mathAppliedConceptState.firstCompletedAt=mathAppliedConceptState.firstCompletedAt||mathAppliedConceptState.completedAt;if(mathAppliedConceptState.relearning){mathAppliedConceptState.history=[...(mathAppliedConceptState.history||[]),{completedAt:attemptedAt,results:mathAppliedConceptState.results}];mathAppliedConceptState.relearning=false;}else if(firstCompletion){mathAppliedConceptState.firstCompletedAt=attemptedAt;}mathAppliedConceptState.phase='completed';if(firstCompletion)addCompletedStudyActivity({source:'math',key:`math_appliedConcept_${mathAppliedConceptUnit.id}`,title:`응용개념학습 · ${mathAppliedConceptUnit.title}`,detail:'완료',completedAt:attemptedAt});}
  const wrong=correct?null:{question,item,answer:String(mathAppliedConceptAnswer),attemptedAt};mathAppliedConceptAnswer='';await saveMathAppliedConceptLearning_(wrong);renderMathAppliedConceptLearning_();
}
async function restartMathAppliedConceptLearning(){const completedAt=mathAppliedConceptState.completedAt,firstCompletedAt=mathAppliedConceptState.firstCompletedAt||completedAt,history=[...(mathAppliedConceptState.history||[])];mathAppliedConceptState={...defaultMathAppliedConceptState_(),completed:true,completedAt,firstCompletedAt,relearning:true,history};mathAppliedConceptAnswer='';await saveMathAppliedConceptLearning_();renderMathAppliedConceptLearning_();}
async function saveMathAppliedConceptLearning_(wrong=null){
  if(isLearningWriteBlocked())return false;
  const name=playerName,unitId=mathAppliedConceptUnit.id;let snapshot=pickNewerMathConceptProgress_(readMathConceptProgressWithMigration_(name,unitId),mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]);if(!snapshot||snapshot.studentKey!==name||snapshot.unitId!==unitId)return false;
  snapshot=JSON.parse(JSON.stringify(snapshot));snapshot.appliedConcepts=JSON.parse(JSON.stringify(mathAppliedConceptState));snapshot.wrongPractice={...(snapshot.wrongPractice||{}),items:{...(snapshot.wrongPractice?.items||{})}};
  if(wrong){const q=wrong.question,old=snapshot.wrongPractice.items[q.id]||{};snapshot.wrongPractice.items[q.id]={...old,questionId:q.id,unitId,conceptId:wrong.item.conceptId,question:q.question,choices:Array.isArray(q.choices)?q.choices.slice():[],correctAnswer:String(q.correctAnswer),acceptedAnswers:Array.isArray(q.acceptedAnswers)?q.acceptedAnswers.slice():[],diagram:String(q.diagram||''),studentAnswer:wrong.answer,explanation:String(q.explanation||''),source:'applied-concept-learning',wrongAt:wrong.attemptedAt,resolved:false,resolvedAt:null};}
  snapshot.syncRevision=(Number(snapshot.syncRevision)||0)+1;snapshot.updatedAt=new Date().toISOString();snapshot.pendingSync=true;try{localStorage.setItem(mathConceptUnitStorageKey_(name,unitId),JSON.stringify(snapshot));}catch(error){}mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;
  for(let attempt=0;attempt<2;attempt++){const result=await apiSaveMathConceptProgress(snapshot);if(result?.ok){snapshot.lastServerRevision=Math.max(Number(snapshot.lastServerRevision)||0,Number(result.savedRevision)||snapshot.syncRevision);snapshot.pendingSync=snapshot.syncRevision>snapshot.lastServerRevision;try{localStorage.setItem(mathConceptUnitStorageKey_(name,unitId),JSON.stringify(snapshot));}catch(error){}mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;renderIncompleteUnitsSection();renderMathLearningCards_();return true;}if(result?.error!=='STALE_REVISION'||!result.data||result.data.unitId!==unitId)break;const server=result.data;snapshot={...server,appliedConcepts:JSON.parse(JSON.stringify(mathAppliedConceptState)),wrongPractice:{...(server.wrongPractice||{}),items:{...(server.wrongPractice?.items||{}),...(snapshot.wrongPractice?.items||{})}},syncRevision:Math.max(Number(result.savedRevision)||0,Number(server.syncRevision)||0)+1,updatedAt:new Date().toISOString(),pendingSync:true};}
  mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;renderIncompleteUnitsSection();renderMathLearningCards_();return false;
}
...
async function openMathBasicConceptReview(unitId){
  ...
  if(__card){ if(__card.classList.contains('tap-pending'))return; __card.classList.add('tap-pending'); }
  try{
    const content=await loadMathConceptContent();
    // 진행 상태는 로그인 시 이미 캐시돼 있으므로 재조회를 기다리지 않고 화면부터 연다 — 최신값은 백그라운드로만 확인.
    ...
    mathBasicReviewUnit=unit;mathBasicReviewItems=unit.basicConceptReview;const saved=eligible.progress.basicConceptReview||{};
    const hadRecord=hasMathBasicReviewRecord_(eligible.progress),retroactive=isMathBasicReviewRetroactive_(eligible.progress,unitId,new Date(),content);
    mathBasicReviewState={createdAt:saved.createdAt||new Date().toISOString(),createdForCompletedAt:saved.createdForCompletedAt||eligible.progress.completedAt,createdForScheduledDate:saved.createdForScheduledDate||unit.scheduledDate,createdMode:saved.createdMode||(retroactive?'retroactive':'scheduled-next-day'),completed:saved.completed===true,completedAt:saved.completedAt||null,firstCompletedAt:saved.firstCompletedAt||saved.completedAt||null,relearning:saved.relearning===true,history:[...(saved.history||[])],results:{...(saved.results||{})}};
    // 최초 기록 저장도 화면 진입을 막지 않도록 백그라운드로 보낸다 — mathBasicReviewState는 이미 메모리에 구성되어 렌더에 필요 없음.
    if(!hadRecord)saveMathBasicConceptReview_(playerName,unitId,mathBasicReviewState).catch(error=>console.warn('기본개념 확인 최초 기록 저장 실패:',error));
    const current=getMathBasicReviewCurrent_();mathBasicReviewAnswer='';mathBasicReviewShowingConcept=!!(current&&mathBasicReviewState.results[current.conceptId]?.needsReview);
    htShowOnlyScreen('math-concept-screen');renderMathBasicConceptReview_();
  }finally{
    if(__card)__card.classList.remove('tap-pending');
  }
}
...
function startMathBasicReviewRetry(){mathBasicReviewShowingConcept=false;mathBasicReviewAnswer='';renderMathBasicConceptReview_();}
async function restartMathBasicConceptReview(){
  const completedAt=mathBasicReviewState.completedAt,firstCompletedAt=mathBasicReviewState.firstCompletedAt||completedAt,history=[...(mathBasicReviewState.history||[])];
  mathBasicReviewState={...mathBasicReviewState,completed:true,completedAt,firstCompletedAt,relearning:true,history,results:{}};
  mathBasicReviewAnswer='';mathBasicReviewShowingConcept=false;await saveMathBasicConceptReview_(playerName,mathBasicReviewUnit.id,mathBasicReviewState);renderMathBasicConceptReview_();
}
async function submitMathBasicReviewAnswer(){
  if(blockExpiredHomeworkStep_(mathBasicReviewUnit?.scheduledDate||''))return;
...
async function saveMathBasicConceptReview_(name,unitId,state,wrong=null){
  if(isLearningWriteBlocked())return false;
  let snapshot=pickNewerMathConceptProgress_(readMathConceptProgressWithMigration_(name,unitId),mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]);if(!snapshot||snapshot.studentKey!==name||snapshot.unitId!==unitId)return false;
  snapshot=JSON.parse(JSON.stringify(snapshot));snapshot.basicConceptReview=JSON.parse(JSON.stringify(state));snapshot.wrongPractice={...(snapshot.wrongPractice||{}),items:{...(snapshot.wrongPractice?.items||{})}};
  if(wrong){const q=wrong.question,old=snapshot.wrongPractice.items[q.id]||{};snapshot.wrongPractice.items[q.id]={...old,questionId:q.id,unitId,conceptId:wrong.conceptId,question:q.question,choices:Array.isArray(q.choices)?q.choices.slice():[],correctAnswer:String(q.correctAnswer),acceptedAnswers:Array.isArray(q.acceptedAnswers)?q.acceptedAnswers.slice():[],diagram:String(q.diagram||''),studentAnswer:wrong.answer,explanation:String(q.explanation||''),wrongAt:old.wrongAt||wrong.attemptedAt,resolved:false};}
  snapshot.syncRevision=(Number(snapshot.syncRevision)||0)+1;snapshot.updatedAt=new Date().toISOString();snapshot.pendingSync=true;
  try{localStorage.setItem(mathConceptUnitStorageKey_(name,unitId),JSON.stringify(snapshot));}catch(error){}mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;
  for(let attempt=0;attempt<2;attempt++){const result=await apiSaveMathConceptProgress(snapshot);if(result?.ok){snapshot.lastServerRevision=Math.max(Number(snapshot.lastServerRevision)||0,Number(result.savedRevision)||snapshot.syncRevision);snapshot.pendingSync=snapshot.syncRevision>snapshot.lastServerRevision;try{localStorage.setItem(mathConceptUnitStorageKey_(name,unitId),JSON.stringify(snapshot));}catch(error){}mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;renderIncompleteUnitsSection();renderMathLearningCards_();return true;}
    if(result?.error!=='STALE_REVISION'||!result.data||result.data.unitId!==unitId)break;const server=result.data;snapshot={...server,basicConceptReview:JSON.parse(JSON.stringify(state)),wrongPractice:{...(server.wrongPractice||{}),items:{...(server.wrongPractice?.items||{}),...(snapshot.wrongPractice?.items||{})}},syncRevision:Math.max(Number(result.savedRevision)||0,Number(server.syncRevision)||0)+1,updatedAt:new Date().toISOString(),pendingSync:true};}
  mathConceptProgressOverviewCache[mathConceptCacheKey_(name,unitId)]=snapshot;renderIncompleteUnitsSection();renderMathLearningCards_();return false;
}
...
let completedReplayUnitAttempt=false;
...
function openStartQuizPopup(){
  const u=UNITS[currentUnit];
  if(isHomeworkPastDue_(playerName,deadlineMap?.[currentUnit]||'')){showToast2('🔒 오늘 학습 시간이 종료되었습니다.');return;}
...
async function refreshHomeHeading(){
  const loaded=await Promise.all([apiListDeadlines(),apiGetAfterMidnightHomeworkAccess()]);deadlineMap=loaded[0]||{};
  scheduleHomeworkMidnightRefresh_();
...
function loadStudentDataIfStale(name, forceRefresh=false, options={}){
  if(!name) return Promise.resolve();
  const now=Date.now();

  // 이전 서버 성공 스냅샷을 먼저 적용해 빈 화면 없이 즉시 표시하고, 아래 조회로 최신화한다.
  const restoredFromCache=options.cacheAlreadyRestored===true||restoreStudentServerSnapshot_(name);
...
async function selectStudent(card,name){
  ...
  // 1) 지연 로드되는 수학 콘텐츠를 먼저 준비해야 학생 캐시의 수학 unitId를 복원할 수 있다.
  // 이 단계 전에는 기존 메모리의 역사 데이터만으로 미완료 목록을 그리지 않는다.
  try{await loadMathConceptContent();}catch(error){console.warn('수학 콘텐츠 선로딩 실패:',error);}
  if(playerName!==name)return;
  const studentCacheRestored=restoreStudentServerSnapshot_(name);

  // 캐시/localStorage로 즉시 홈을 표시 (학생 선택 화면의 4명 카드 재렌더링은 홈에 불필요)
...
function createKingOrderCard(){
  const card=document.createElement('div');
  const completed=playerName&&calculateKingOrderProgress(playerName).completed===true;
  card.className='unit-card'+(completed?' done':'');
  card.dataset.learningContent='kingOrder';
  card.innerHTML=`<div class="unit-icon">👑</div>
    <div class="unit-info">
      <div class="unit-title">역대 왕 계보</div>
      <div class="unit-sub">${completed?'완료 · 다시 학습하기':KING_ORDER_DUE_LABEL+' · 핵심 왕만 문제'}</div>
    </div>`;
...
function renderKingOrderList(){
  ...
  body.innerHTML=`<div class="king-era-grid">${eras.map(era=>{
    ...
    return `<button class="king-era-card ${done?'done':''}" onclick="openKingOrderEra('${era.id}')">
      ...
      <span class="king-era-status">${done?'완료 · 다시 학습하기':'시작 →'}</span>
    </button>`;
  }).join('')}</div>`;
}
...
function createHistoryTrainingCard(){
  const c=document.createElement('div');
  const completed=playerName&&calculateHistoryTrainingOverallProgress(playerName).completed===true;
  c.className='unit-card'+(completed?' done':'');
  c.dataset.learningContent='historyTraining';
  c.innerHTML=`<div class="unit-icon">🏛️</div>
    <div class="unit-info">
      <div class="unit-title">역사 훈련소</div>
      <div class="unit-sub">${completed?'완료 · 다시 학습하기':'읽고 쓰며 완성하는 한국사 복습'}</div>
    </div>`;
  ...
  return `<div class="history-summary-divider">
    ...
    ${summaries.map(({config,prog})=>`<div class="history-summary-card${prog.completed?' done':''}" style="margin-bottom:10px" onclick="openHistorySummary1('${config.id}')">
      ...
      <div class="history-summary-info">
        ...
        <span>${prog.completed?'완료 · 다시 학습하기':'읽기 전용 · 1분 핵심요약 포함'}</span>
      </div>
      <button class="history-summary-btn" onclick="event.stopPropagation();openHistorySummary1('${config.id}')">${prog.completed?'다시 학습하기':'시작하기'}</button>
    </div>`).join('')}
  </div>`;
}
...
function renderHistoryTrainingList(){
  ...
  body.innerHTML=visibleParts.map(part=>{
    ...
    const btnLabel=done?'완료 · 다시 학습하기':getResumeButtonLabel(progress);
...
function htShowFirstResult(part){
  ...
  if(wrong===0){
    ...
    prog.completed=true;
    prog.completedAt=prog.completedAt||new Date().toLocaleString('ko-KR');
...
function startQuiz(){
  ...
  completedReplayUnitAttempt=(UNITS[currentUnit]?.examMode?calculateSummaryQuizProgress(playerName,currentUnit):calculateUnitProgress(playerName,currentUnit)).completed===true;
  resetMingeonMemoryCardFlow();
  submitted=false;
  startNewQuizAttemptContext_(); // 실제 새 퀴즈 시작 — 새 resultId 발급(재렌더링이 아니라 진짜 시작이므로)
  const unit=UNITS[currentUnit];
  const quizScreen=document.getElementById('quiz-screen');if(quizScreen)quizScreen.dataset.homeworkDueDate=deadlineMap?.[currentUnit]||'';
  if(!isParentUnitPracticeMode()&&!completedReplayUnitAttempt){
...
async function submitExam(auto){
  ...
  document.getElementById('result-msg').textContent=isParentUnitPracticeMode()
    ...
    :completedReplayUnitAttempt?'완료 기록은 유지됩니다. 다시 학습한 결과는 완료 알림을 보내지 않아요.':playerName+'님 [정리문제], '+msg;
  ...
  if(completedReplayUnitAttempt){
    submitted=true;
    document.getElementById('submit-btn').disabled=true;
    document.getElementById('submit-btn').textContent='✅ 다시 학습 완료 · 기존 기록 유지';
    const actions=document.getElementById('result-main-actions');
    if(actions)actions.style.display=wrongQs.length?'none':'flex';
    if(wrongQs.length>0)forcedReviewTimer=setTimeout(()=>beginExamWrongReview(wrongQs),1100);
    return;
  }
...
async function openMathConceptLearning(unitId){
  ...
  try{
    ...
    // 진행 상태는 로그인 시 이미 캐시돼 있으므로 재조회를 기다리지 않고 화면부터 연다 — 최신값은 백그라운드로만 확인.
    loadMathConceptProgressForName_(playerName).catch(error=>console.warn('수학개념학습 진행 상태 백그라운드 갱신 실패:',error));
    const lock=getMathSequentialLock_(playerName,targetUnitId,content);
    if(lock.locked){showToast2('🔒 이전 학습을 먼저 완료해 주세요');renderMathLearningCards_();return;}
    return await window.MathFlowV2.open(targetUnitId);
  }finally{
    if(__card)__card.classList.remove('tap-pending');
  }
}
function closeMathConceptLearning(){const r=window.MathFlowV2?.close();updateProgressColors();return r;}
// 홈 "미완료 학습" 목록의 "새 학습 이어하기" 카드 전용 진입점 — 완료 화면(다시 학습하기/새 학습 이어하기 선택)을
// 거치지 않고 곧바로 확장 학습으로 들어간다. startExtensionLearning()은 완료 상태가 아니거나 새 항목이 없으면
// 조용히 아무 것도 하지 않도록 이미 구현돼 있어(math-flow-v2.js), 항상 안전하게 호출할 수 있다.
async function openMathConceptLearningExtension(unitId){
  await openMathConceptLearning(unitId);
  if(typeof window.MathFlowV2?.startExtensionLearning==='function')window.MathFlowV2.startExtensionLearning();
}
